...
# Recent-high context for the SPX-vs-breadth divergence test. We reuse a wider
# SPY history (range=6mo) because the existing _spy_hist uses range=10d.
# NOTE (flagged): the legacy 200DMA gate above calls yahoo_closes("SPY", 205)
# which is hard-capped at range=10d, so it can never see 200 sessions and the
# gate is effectively dormant. Left as-is (out of scope); see PR description.
_spx_near_high = None
try:
    _spy_wide = _yahoo_closes_range("SPY", "6mo")
    if _spy_wide and len(_spy_wide) >= 20 and spy_px is not None:
        _recent_high = max(_spy_wide[-60:]) if len(_spy_wide) >= 60 else max(_spy_wide)
        # "at/near recent highs" = within 2% of the recent high
